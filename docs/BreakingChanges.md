...
- The test tree now appears in VS Code's built-in Testing view (the flask icon in the Activity Bar), not the third-party Test Explorer view.
...
- Any settings or keybindings that specifically targeted the Test Explorer UI's own commands/views no longer apply — use VS Code's native Testing view/commands instead.
